...
body_sphere = builder.add_body(
    xform=wp.transform(sphere_pos, wp.quat_identity(dtype=float)),  # type: ignore
    key="sphere",
)
...
body_box = builder.add_body(
    xform=wp.transform(box_pos, wp.quat_identity(dtype=float)),  # type: ignore
    key="box",
)
...
body_capsule = builder.add_body(
    xform=wp.transform(capsule_pos, wp.quat_identity(dtype=float)),  # type: ignore
    key="capsule",
)
