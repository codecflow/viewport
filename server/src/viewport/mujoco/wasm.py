...
def _resolve_asset_path(
    xml_base_dir: Path | None, asset_dir: str | None, filename: str
) -> Path | None:
...
def _copy_external_assets(
    xml_content: str, xml_base_dir: Path | None, output_dir: Path
) -> tuple[list[str], list[str]]:
    """Copy external assets by parsing XML content."""
    print("Copying external assets from XML...")

    meshdir, texturedir, mesh_files_xml, texture_files_xml = _parse_xml_assets(
        xml_content
    )
    ...
    if texture_files_xml:
        ...
        for texture_filename in texture_files_xml:
            source_path = _resolve_asset_path(
                xml_base_dir, texturedir, texture_filename
            )
