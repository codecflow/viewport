...
def mesh_geom_to_trimesh(model: mujoco.MjModel, geom_id: int, 
...
def body_geom_ids(model: mujoco.MjModel, body_id: int, 
                  visual_only: bool = True) -> list[int]:
...
def _extract_texture(model: mujoco.MjModel, texid: int, verbose: bool = False) -> Image.Image | None:
...
def _default_color(model: mujoco.MjModel, geom_id: int) -> np.ndarray:
    ...
    is_collision = (model.geom_contype[geom_id] != 0 or 
